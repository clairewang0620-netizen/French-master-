    ...
    return (
      <div className="flex flex-col h-[calc(100vh-100px)] space-y-4">
        <div className="flex items-center gap-3">
          <button onClick={exitFlashCard} className="text-slate-400 p-2"><ArrowLeft size={20} /></button>
          <div className="h-1.5 flex-1 bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-brand-500" style={{ width: `${((flashCardIndex + 1) / filteredWords.length) * 100}%` }} />
          </div>
        </div>

        <div className="flex-1 bg-white rounded-card shadow-lg p-6 flex flex-col items-center justify-center text-center">
          <h2 className="text-4xl font-black text-slate-900">{word.fr}</h2>
          ...
          <div className="w-full mt-8 space-y-3 text-left overflow-y-auto max-h-48 px-2">
            ...
            {word.examples.map((ex, i) => (
              <div key={i} className="p-3 bg-slate-50 rounded-xl flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-800">{ex.fr}</p>
                  <p className="text-[11px] text-slate-500 mt-0.5">{ex.cn}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-4 gap-2 py-4">
           ...
           <button onClick={nextCard} className="col-span-2 h-12 bg-slate-900 text-white rounded-btn font-black flex items-center justify-center gap-2">
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {['A1', 'A2', 'B1', 'B2', 'C1'].map(lvl => (
          <button 
            ...
            className={clsx("px-5 py-2 rounded-full font-black text-xs border", selectedLevel === lvl ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-400 border-slate-100")}
          >
            {lvl}
          </button>
        ))}
      ...
      <div className="grid gap-2">
        {filteredWords.map((word, i) => (
          <button key={word.id} onClick={() => enterFlashCard(i)} className="bg-white p-4 rounded-xl border border-slate-50 shadow-sm flex items-center justify-between">
            <div className="text-left">
              <p className="font-bold text-slate-800">{word.fr}</p>
