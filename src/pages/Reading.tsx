...
import { ArrowLeft, BookOpen, Languages, Pause, Play, Sparkles } from 'lucide-react';
    ...
    return (
      <div className="space-y-4 animate-in slide-in-from-bottom-4 pb-20">
        <header className="flex items-center gap-3 sticky top-0 bg-white/95 backdrop-blur-md py-2 z-10">
          <button onClick={() => { setActiveId(null); tts.stop(); }} className="p-1.5 bg-slate-50 rounded-full text-slate-400"><ArrowLeft size={20} /></button>
        ...
        <article className="bg-white rounded-card p-6 shadow-md border border-slate-50 space-y-6">
          ...
          <div className="flex justify-center border-b border-slate-50 pb-8 pt-4">
            <button 
              ...
              className={clsx(
                ...
                playbackStatus === 'playing' ? "bg-[#FF4C4C]" : "bg-[#7CFC00]"
              )}
            >
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <p className="text-lg leading-[2] text-slate-800 font-medium tracking-wide">
                {article.content_fr}
              </p>
              {showZH && (
                <div className="p-4 rounded-2xl bg-slate-50 border-l-4 border-brand-200 text-slate-500 text-sm italic leading-relaxed animate-in fade-in">
                  {article.content_zh}
                </div>
              )}
            </div>
          ...
          <div className="flex flex-col gap-3 pt-6 border-t border-slate-50">
            <button onClick={() => setShowZH(!showZH)} className="flex items-center justify-center gap-2 bg-slate-50 text-slate-600 h-12 rounded-btn font-black text-xs shadow-inner">
              <Languages size={18} />
            ...
            {article.keywords && article.keywords.length > 0 && (
              <section className="bg-brand-50 rounded-card p-5 space-y-4">
                <div className="flex items-center gap-2 text-brand-600">
                  <Sparkles size={18} />
                  <h3 className="font-black text-[11px] uppercase tracking-wider">核心词汇</h3>
                </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3">
        {readingData.map(art => (
          <button key={art.id} onClick={() => setActiveId(art.id)} className="bg-white p-5 rounded-card border border-slate-100 shadow-sm flex flex-col gap-2 relative overflow-hidden group">
            ...
            <h3 className="text-base font-black text-slate-800 group-hover:text-brand-500 transition-colors">{art.title}</h3>
            <p className="text-slate-400 text-xs line-clamp-1">{art.content_fr.slice(0, 80)}...</p>
          </button>
        ))}
      </div>
    </div>
